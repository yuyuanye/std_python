...
class Block(Button):   #块类  
    ...
    def __init__(self,p,blockType,master,r,bm):
        ...
        #self.bind("<ButtonPress>",btn_MouseDown);
        self.place(x=self.Location.X*110,y=self.Location.Y*110)
    '''
      GetPoints()方法获取块中所有点
...
def btn_Realse(event):
    ...
    x=int(event.widget.place_info()["x"])//110
    y=int(event.widget.place_info()["y"])//110
    ...
    if (moveH >= BlockSize * 1 / 3):
        ...
        return
    event.widget.place(x=block.Location.X*110,y=block.Location.Y*110) 
...
win = Tk();#创建窗口对象
win.title("华容道游戏--夏敏捷3-14")#设置窗口标题
win.geometry("440x550")

game=Game()
bm = [PhotoImage(file = 'bmp\\曹操.png'),
      PhotoImage(file = 'bmp\\关羽.png'),
      PhotoImage(file = 'bmp\\黄忠.png'),
      PhotoImage(file = 'bmp\\马超.png'),
      PhotoImage(file = 'bmp\\张飞.png'),
      PhotoImage(file = 'bmp\\赵云.png'),
