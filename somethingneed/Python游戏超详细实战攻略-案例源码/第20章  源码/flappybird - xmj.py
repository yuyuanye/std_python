...
class Pipeline(object):
    ...
    def __init__(self):
        ...
        self.wally_Up = random.random()*100-350     # 上管道所在y轴坐标
        self.wally_Down = 500-random.random()*100   # 下管道所在y轴坐标
        self.pineUp   = pygame.image.load("assets/top.png")
        self.pineDown = pygame.image.load("assets/bottom.png")
        
    def pineLineUpdate(self):
        """"管道移动方法"""
        ...
        if self.wallx < -80: 
            ...
            self.wallx = 400
            self.wally_Up = random.random()*100-350     # 上管道所在y轴坐标
            self.wally_Down = random.random()*50+350  # 下管道所在y轴坐标
...
def createMap():
    ...
    # 显示管道
    screen.blit(Pipeline.pineUp,(Pipeline.wallx,Pipeline.wally_Up));    # 上管道坐标位置 
    screen.blit(Pipeline.pineDown,(Pipeline.wallx,Pipeline.wally_Down)); # 下管道坐标位置
    Pipeline.pineLineUpdate()      # 管道移动
...
def checkDead():
    # 上方管子的矩形位置
    upRect = pygame.Rect(Pipeline.wallx,Pipeline.wally_Up,
                         Pipeline.pineUp.get_width() - 10,
                         Pipeline.pineUp.get_height())

    # 下方管子的矩形位置
    downRect = pygame.Rect(Pipeline.wallx,Pipeline.wally_Down,
